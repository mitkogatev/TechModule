...
namespace _06.BookLibraryModification
{
    class BookLibraryModification
    {
        static void Main(string[] args)
        {
            ...
            foreach (var title in myLib.ReleasesAfterDate(releaseDate).OrderBy(t=>t.Key).OrderBy(t=>t.Value))
            {
    ...
    class Library
    {
        ...
        public Dictionary<string, DateTime> ReleasesAfterDate(DateTime date)
        {
            Dictionary<string,DateTime> result = Books.Where(b => b.ReleaseDate > date).ToDictionary(b =>b.Title,b=>b.ReleaseDate);
            return result;
        }
    }
}
